...
public:
    explicit SmbOrchestrator(size_t num_tuples, size_t num_threads) : morsel_creator(num_tuples), page_manager(), num_threads(num_threads) {
    }
    ...
    void run() {
        ...
        for (unsigned i = 0; i < num_threads; i++) {
            threads.emplace_back([this] {
                process_morsel_smb<T, partitions, page_size>(morsel_creator, page_manager, num_threads);
            });
        }
    }
